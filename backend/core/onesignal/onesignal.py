...
ONESIGNAL = settings.ONESIGNAL


class Onesignal:
    def __init__(
            self,
            app_id: str = ONESIGNAL.get('app_id'),
            rest_api_key: str = ONESIGNAL.get('rest_api_key'),
            user_auth_key: str = ONESIGNAL.get('user_auth_key'),
    ...
    def create_notification(self, body: Dict) -> Response:
        ...
        path = self.api_root + NOTIFICATIONS_PATH
        payload = {"app_id": self.app_id, **body}
        return requests.post(path, headers=header, data=json.dumps(payload))
